...
class device():
    # FIX #12: Class-level buffer'lar tüm device instance'ları ve thread'ler
    # arasında paylaşılıyordu → race condition (tuş düşmesi/takılması).
    # Artık __init__ içinde per-instance olarak oluşturuluyor.
    handle = 0
    event = 0
    is_keyboard = False
    ...
    def __init__(self, handle, event, is_keyboard: bool):
        self._parser = None
        self._bytes_returned = (c_int * 1)(0)
        self._c_byte_500 = (c_byte * 500)()
        self._c_int_2 = (c_int * 2)()
        self._c_ushort_1 = (c_ushort * 1)()
        self._c_int_1 = (c_int * 1)()
        self._c_recv_buffer = None
