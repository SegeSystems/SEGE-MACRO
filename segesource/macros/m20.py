...
def _default_send_key(scan_code: int):
    if _ClicksendKeyboardDriver is None:
        print(f"[M20][FAKE] {scan_code:#04x}")
        return
...
class M20Macro:
    """
    M20 MACRO: F-bar seç -> skill tuşuna bas.
    """
    def __init__(
        self,
        f_bar: int = 3,
...
# =========================================================
# 2. GUI (AYAR PENCERESİ) - Main.py'den Birebir
# =========================================================
class M20SettingsDialog(QDialog):
    def __init__(self, parent=None, current_config=None):
        super().__init__(parent)
        self.setWindowTitle("M20 AYARLARI")
        self.setModal(True)
        self._capture_next_key = False
        if current_config is None: current_config = {}

        f_bar = int(current_config.get("f_bar", 3))
        digit = int(current_config.get("digit", 4))
        mode = current_config.get("mode", "toggle")
        hotkey = current_config.get("hotkey", "M")
        ...
        l.addWidget(QLabel("SKILL BAR (F1–F9):"), 0, 0)
        self.f = QComboBox()
        
        # 1. "F YOK" seçeneğini ekle (Değer olarak 0 tutar)
        self.f.addItem("F YOK", 0)
        
        # 2. F1'den F9'a kadar döngüyle ekle
        for i in range(1, 10):
            self.f.addItem(f"F{i}", i) # Görünen: F1, Saklanan Değer: 1
        
        # 3. Kayıtlı değeri config'den bul ve seç
        idx = self.f.findData(f_bar)
        if idx < 0: idx = 0 # Eğer kayıtlı veri yoksa veya 0 ise F YOK seç
        self.f.setCurrentIndex(idx)
        
        l.addWidget(self.f, 0, 1)

        l.addWidget(QLabel("SKILL TUŞU (0–9):"), 1, 0)
        self.d = QComboBox(); self.d.addItems([str(i) for i in range(10)])
        self.d.setCurrentText(str(digit))
        l.addWidget(self.d, 1, 1)

        l.addWidget(QLabel("M20 MODU:"), 2, 0)
...
# 1. M20Macro sınıfının içine şu metodu ekleyin (set_f_bar vb. metotların yanına):
    def update_config(self, cfg):
        with self._lock:
            self.f_bar = int(cfg.get("f_bar", 3))
            self.digit = int(cfg.get("digit", 4))
            self.mode = cfg.get("mode", "toggle")
            self.combo_delay = max(0.0, float(cfg.get("combo_delay", 0.05)))

# 2. M20Widget sınıfının içindeki 'settings' fonksiyonunu şununla değiştirin:
    def settings(self):
        dlg = M20SettingsDialog(self, self.config)
        if dlg.exec_() == QDialog.Accepted:
            self.config.update(dlg.result_config)
            
            # ESKİSİ: Tek tek set ediyorduk
            # YENİSİ: Tek seferde update_config
...
# =========================================================
# --- m20.py --- (Sadece M20Widget sınıfını değiştirin)

# ... (Macro ve SettingsDialog kodları yukarıda kalır) ...

# =========================================================
# 3. WIDGET (ANA EKRAN KARTI)
# =========================================================
# --- m20.py --- (M20Widget Sınıfının Tamamı)

class M20Widget(QFrame):
    ...
    def setup_ui(self):
        ...
        r = QHBoxLayout()
        r.setSpacing(4)
        r.addWidget(QLabel("AKTİF TUŞ:"))
        
        self.hk = QLabel(self.config.get("hotkey", "M"))
    ...
    def toggle_listen(self):
        ...
        if not self.listen_active:
            k = self.config.get("hotkey", "M").lower()
    ...
    def _safe_update_status(self):
        if not self.listen_active:
            ...
            # Makro çalışıyor mu kontrolü
            # (Bazı makrolarda is_running property, bazılarında attribute olabilir, hepsiyle uyumlu)
        ...
        # Buton rengini güncelle (Yeşil/Kırmızı çerçeve için)
