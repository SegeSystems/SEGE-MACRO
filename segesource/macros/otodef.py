...
class OtoDefMacro:
    def __init__(self):
        # Tarama yapılacak resimler
        self.skills = {
            "800": {"enabled": False, "f": 1, "d": 1, "icon": "def800.png", "tmpl": None, "last_cast": 0},
            "400": {"enabled": False, "f": 1, "d": 2, "icon": "def400.png", "tmpl": None, "last_cast": 0},
            "200": {"enabled": False, "f": 1, "d": 3, "icon": "def200.png", "tmpl": None, "last_cast": 0},
        }
        self.scan_order = ["800", "400", "200"]
    ...
    # --- DÜZELTME BURADA: ARTIK TEK BİR 'cfg' SÖZLÜĞÜ ALIYOR ---
    def update_config(self, cfg):
        with self._lock:
            # Gelen config sözlüğünden 'skills' kısmını okuyoruz
            if "skills" in cfg:
            ...
            # Bölgeyi her güncellemede yeniden yükle
            self._load_buff_region()
    # -----------------------------------------------------------
    ...
    def _load_templates(self):
        base_dir = "icons" # veya icons klasörü, ikonlarının nerede olduğuna dikkat et
        # Eğer ikonlar 'icons' içindeyse burayı "icons" yapabilirsin.
        # Genelde oto def ikonları: def800.png vs.
        ...
        for key, data in self.skills.items():
            ...
            if os.path.isfile(path):
                try:
                    ...
                    if img is not None:
                        self.skills[key]["tmpl"] = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                except: pass
    ...
    def start(self):
        ...
        if not self.buff_region:
            print("[DEF] Buff bölgesi ayarlanmamış!")
            return
        ...
        self._running = True
        print("[DEF] Başlatıldı.")

    def stop(self):
        self._stop_event.set()
        self._running = False
        print("[DEF] Durduruldu.")

    def _loop(self):
        sct = mss.mss()
        # Eşik değerini biraz düşürerek algılamayı kolaylaştırıyoruz (Eski dosyada 0.70 idi)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                for key in active_skills:
                    ...
                    # Eğer skor düşükse (Simge bulunamadıysa)
                    if max_val < self.match_threshold:
                        
                        # --- EKLENEN KISIM: YANIP SÖNME KORUMASI ---
                        is_really_gone = True
                        # 5 kere daha kontrol et, belki sadece yanıp sönüyordur veya mouse üstündedir.
                        for _ in range(3):
                            time.sleep(0.05) 
                            if self._stop_event.is_set(): 
                                is_really_gone = False
                                break
                            
                            # Tekrar ekran alıp bakıyoruz
                            check_scr = np.array(sct.grab(self.buff_region))
                            check_gray = cv2.cvtColor(check_scr, cv2.COLOR_BGRA2GRAY)
                            check_res = cv2.matchTemplate(check_gray, skill["tmpl"], cv2.TM_CCOEFF_NORMED)
                            _, check_val, _, _ = cv2.minMaxLoc(check_res)
                            
                            # Eğer bu kontrollerin birinde simge görünürse basmaktan vazgeç
                            if check_val >= self.match_threshold:
                                is_really_gone = False
                                print(f"[EXPLORE] {key} anlık kayboldu (Skor: {check_val:.2f}), basılmadı.")
                                break

                        if is_really_gone:
                            print(f"[EXPLORE] {key} kesin bitti (Skor: {max_val:.2f}). Basılıyor...")
                            
                            f_val = skill["f"]
                            if f_val > 0:
                                f_code = F_SCANCODES.get(f_val)
                                if f_code: 
                                    _default_tap(f_code, 0.02)
                                    time.sleep(0.05)
                            
                            d_code = DIGIT_SCANCODES.get(skill["d"])
                            if d_code: 
                                _default_tap(d_code, 0.02)
...
# =========================================================
class OtoDefSettingsDialog(QDialog):
    def __init__(self, parent, config, macro):
        super().__init__(parent)
        self.setWindowTitle("OTO DEF AYARLARI")
        self.setModal(True)
        ...
        self.txt_hotkey.setText(self.config.get("hotkey", "A").upper())
        ...
        self.skill_rows = [
            ("800", "800 DEF", "icons/def800.png"),
            ("400", "400 DEF", "icons/def400.png"),
            ("200", "200 DEF", "icons/def200.png"),
        ]
    ...
    def accept(self):
        # 1. Config dictionary'i GUI'den güncelle
        ...
        # 2. Makroya TOPLU güncelleme gönder
...
# =========================================================
class OtoDefWidget(QFrame):
    update_signal = pyqtSignal()

    def __init__(self, parent=None, macro_instance=None, config=None):
        super().__init__(parent)
        self.macro = macro_instance or OtoDefMacro()
        self.config = config or {}

        # Config'i makroya yükle
        self.macro.update_config(self.config)
    ...
    def setup_ui(self):
        ...
        icons = ["def800.png", "def400.png", "def200.png"]
        for icon in icons:
            l = QLabel(); l.setFixedSize(25, 25); pix = QPixmap(f"icons/{icon}")
            if not pix.isNull(): l.setPixmap(pix.scaled(25, 25, Qt.KeepAspectRatio, Qt.SmoothTransformation))
            h.addWidget(l)
        h.addStretch(1); lbl = QLabel("OTO DEF"); lbl.setObjectName("MinorHeaderLabel"); h.addWidget(lbl)
        v.addLayout(h)
        ...
        self.lbl_hotkey = QLabel(self.config.get("hotkey", "A")); self.lbl_hotkey.setObjectName("HotkeyLabel"); self.lbl_hotkey.setAlignment(Qt.AlignCenter)
    ...
    def open_settings(self):
        dlg = OtoDefSettingsDialog(self, self.config, self.macro)
        if dlg.exec_() == QDialog.Accepted:
            self.lbl_hotkey.setText(self.config.get("hotkey", "A").upper())
    ...
    def toggle_listen(self):
        ...
        if not self.listen_active:
            hotkey = self.config.get("hotkey", "a").lower()
            try:
                self._hotkey_hook = keyboard.on_press_key(hotkey, self.on_hotkey_press, suppress=False)
                self.listen_active = True
            except Exception as e:
                print(f"[DEF] Tuş hatası: {e}"); self.listen_active = False
        else:
    ...
    def on_hotkey_press(self, e):
        self.macro.toggle(); self.update_signal.emit()
