...
def _default_send_key(scan_code: int):
    if _ClicksendKeyboardDriver is None:
        print(f"[OTOBICAK][FAKE] {scan_code:#04x}")
        return
...
class OtoBicakMacro:
    """
    OTO BICAK MACRO: F-bar seç -> skill tuşuna bas.
    Genellikle çiftleme veya yedek silah değişimi için.
    """
    def __init__(
        self,
        f_bar: int = 3,
...
# 1. OtoBicakMacro sınıfının içine şu metodu ekleyin:
    def update_config(self, cfg):
        with self._lock:
            self.f_bar = int(cfg.get("f_bar", 3))
            self.digit = int(cfg.get("digit", 6))
            self.mode = cfg.get("mode", "toggle")
            self.combo_delay = max(0.0, float(cfg.get("combo_delay", 0.05)))

# 2. OtoBicakWidget sınıfının içindeki 'settings' fonksiyonunu şununla değiştirin:
    def settings(self):
        dlg = OtoBicakSettingsDialog(self, self.config)
...
class OtoBicakSettingsDialog(QDialog):
    def __init__(self, parent=None, current_config=None):
        ...
        # 1. "F YOK" seçeneğini ekle (Arkada 0 değerini tutar)
        self.f.addItem("F YOK", 0)
        
        # 2. F1'den F9'a kadar döngüyle ekle
        for i in range(1, 10):
            self.f.addItem(f"F{i}", i) # Görünen: "F1", Değer: 1
        
        # 3. Kayıtlı değeri bul ve seç
        # f_bar değişkeni sayı (int) olarak geliyorsa direkt bulur
        index = self.f.findData(f_bar)
        if index != -1:
            self.f.setCurrentIndex(index)
        else:
            self.f.setCurrentIndex(0) # Bulamazsa F YOK seç
            
        l.addWidget(self.f, 0, 1)
        
        l.addWidget(QLabel("BIÇAK TUŞU (0–9):"), 1, 0)
...
# =========================================================
# --- otobicak.py --- (Sadece OtoBicakWidget sınıfını değiştirin)
...
# 3. WIDGET (ANA EKRAN KARTI)
# ---------------------------------------------------------
# --- otobicak.py --- (OtoBicakWidget Sınıfının Tamamı)

class OtoBicakWidget(QFrame):
    ...
    def setup_ui(self):
        ...
        self.hk = QLabel(self.config.get("hotkey", "B"))
    ...
    def toggle_listen(self):
        ...
        if not self.listen_active:
            k = self.config.get("hotkey", "B").lower()
